...
export function registerStatusBar(context: vscode.ExtensionContext, provider: CsvEditorProvider): void {
  const item = vscode.window.createStatusBarItem("csvEditor.status", vscode.StatusBarAlignment.Right, 100);
  item.name = "CSV";
  ...
  const update = (): void => {
    ...
    if (session) {
      ...
      item.command = "csvEditor.showActions";
      item.show();
      return;
    }
    ...
    if (editor && (isCsvLikeUri(editor.document.uri) || editor.document.languageId === "csv" || editor.document.languageId === "tsv")) {
      ...
      item.command = "csvEditor.openGrid";
      item.show();
      return;
    }
